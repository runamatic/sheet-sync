...
function convertToText(data) {
  ...
  data.forEach(row => {
    ...
    // Si es una fila con varios valores, procesamos los datos como productos
      ...
      const name = row[0] || 'N/A'; // Nombre
      const stock = row[1] || 'N/A'; // Stock

      // Verifica si el precio ya contiene "$" y lo formatea correctamente
      const price = row[2]
        ? (row[2].toString().includes('$') ? row[2] : `$${row[2]}`)
        : '-';

      // Formato del texto
      const rowText = `Nombre: ${name}, Stock: ${stock}, Precio: ${price}`;
